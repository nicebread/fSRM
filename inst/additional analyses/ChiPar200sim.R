...
# simulate data with different sample sizes (H0): partnereffects are equal
SRM_sim <- '
...
for(i in 1:N) {
  ...
  fit_par <- lavaan(SRM_par, data=sim.data)
  chi25 <- c(chi25, anova(fit,fit_par)[2,5])
  p25 <- c(p25, anova(fit,fit_par)[2,7])
}

TI_par25 <- length(chi25[chi25>7.815])
parTI25 <- cbind(chi25, p25, TI_par25)
write.table(parTI25, "chi_par_TI25.csv")
...
for(i in 1:N) {
  ...
  fit <- lavaan(SRM, data=sim.data)
  fit_par <- lavaan(SRM_par, data=sim.data)
  chi50 <- c(chi50, anova(fit,fit_par)[2,5])
  p50 <- c(p50, anova(fit,fit_par)[2,7])
}

TI_par50 <- length(chi50[chi50>7.815])
parTI50 <- cbind(chi50, p50, TI_par50)
write.table(parTI50, "chi_par_TI50.csv")
...
for(i in 1:N) {
  ...
  fit <- lavaan(SRM, data=sim.data)
  fit_par <- lavaan(SRM_par, data=sim.data)
  chi100 <- c(chi100, anova(fit,fit_par)[2,5])
  p100 <- c(p100, anova(fit,fit_par)[2,7])
}

TI_par100 <- length(chi100[chi100>7.815])
parTI100 <- cbind(chi100, p100, TI_par100)
write.table(parTI100, "chi_par_TI100.csv")
